...
Estado al 2026-09-04: **P5 en curso, con el primer bloque visual cerrado 3/3**. `email-enquiries` dispone de evidencia ES/EN exacta en Nivora: tres solicitudes ficticias actualizan una vista previa solo en memoria, sin campos personales, formulario, destinatario real, email, inventario, reserva, almacenamiento o escritura. `website-editor` dispone de evidencia ES/EN exacta en Terrava: Recepción prepara o descarta un titular ficticio y solo Dirección puede aprobar la vista local; recargar restaura el fixture y no existe CMS, repositorio, despliegue, proveedor, persistencia o escritura HTTP. `automation` dispone de evidencia ES/EN exacta en Aurem mediante `?vista=automations`: tres reglas ficticias muestran disparador, condición, resultado y revisión humana local, pero nunca se activan ni ejecutan. Permanece separada del Copiloto en `?vista=automation` y no inicia jobs, colas, cron, webhooks, mensajes, proveedores, persistencia o escritura HTTP. Básico conserva cero dashboard. P5 continúa con contratos de preparación y validación por integración; ninguna conexión está activa.
...
Continuar el segundo bloque de **P5 · Conectividad demostrable y madurez real** definiendo un contrato de preparación para Canales, sin conectar aún ningún proveedor. El contrato debe centralizar propietario, permisos, referencia de credenciales sin valores, mapeo, casos sandbox, idempotencia y reconciliación, fallos y reintentos, auditoría, criterios de aceptación, kill switch y reversión por proveedor ficticio. `?vista=channels` podrá presentar esa preparación en ES/EN conservando cero conexiones, publicación bloqueada y el estado `activable_por_proyecto`. No se añadirán OAuth, secretos, webhooks, adaptadores, logos o estados activos, inventario, tarifas, reservas, mensajes o escrituras externas. Una marca o conexión solo podrá mostrarse como activa después de autorización, contrato y validación técnica real por proveedor.
