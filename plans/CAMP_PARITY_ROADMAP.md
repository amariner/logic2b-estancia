...
| Recorrido | Modal o ruta dedicada | Recorridos dentro de demos | Crear un recorrido comercial transversal de tres minutos que una web, Gestión e Inteligente |
...
Estado al 2026-09-04: **P6 en curso; `web_view`, `panel_view`, `web_handoff` y `panel_handoff` cerrados**. Las doce direcciones web y las seis fichas de panel disponen de rutas comerciales ES/EN allowlisted que emiten una sola vista por carga únicamente tras consentimiento explícito y runtime analítico `live`. Sus acciones reales hacia demo exacta, diagnóstico contextual o el formulario único de contacto emiten además handoffs con destino cerrado, sin retrasar la navegación. Identificador, plan, idioma, origen y destino forman combinaciones cerradas; URL, título, texto, usuario, sesión y PII se rechazan. El informe `2.2.0` admite periodos formados solo por vistas o handoffs y desglosa recuentos agregados por concepto/superficie, destino e idioma. Las demos canónicas conservan CSP `connect-src 'none'`, no leen consentimiento, no consultan capacidades y no cargan GTM; no se relajó su aislamiento para medirlas. GTM, línea base y experimentos continúan bloqueados como actividades externas.
...
Continuar **P6 · Medición y optimización** definiendo primero una superficie comercial verificable que permita empezar y terminar explícitamente un recorrido completo. Debe encadenar evidencia pública ya existente —sin duplicar demos ni inventar estados—, ofrecer controles inequívocos de inicio y final y conservar contexto allowlisted entre pasos. Solo entonces se instrumentarán `tour_start` y `tour_complete`, nunca a partir de scroll, tiempo en página o navegación implícita. Las demos conservarán su CSP `connect-src 'none'` y sus eventos locales sin proveedor. El informe deberá incorporar el recorrido agregado y cubrir consentimiento previo/en página, revocación, idempotencia, abandono sin final y payloads adversarios. GTM, línea base y experimentos siguen bloqueados hasta autorización externa.
