...
- Alcance: transporte del formulario comercial en la landing, copia localizada ES/EN, una prueba E2E determinista y este checkpoint. No se añadieron dependencias, imágenes, endpoints, proveedores, PII, eventos analíticos ni despliegues.
- Conversión y resiliencia: un `AbortController` acota a 15 segundos la petición completa al Worker. Si la red queda bloqueada, el visitante sale de `Enviando…`, conserva los campos y recibe un mensaje reintentable específico sin afirmación de entrega.
- Idempotencia y recuperación: el recibo permanece oculto hasta obtener `delivered` o `delivered_degraded`; la prueba repite el formulario después del timeout y confirma una entrega posterior. El mismo payload continúa resolviéndose con fingerprint, referencia durable y claves estables durante 24 horas.
- Seguridad y privacidad: el temporizador se limpia en éxito o fallo y no registra ni persiste payload, PII, secretos o respuestas. Destinatario, rate limit, consentimiento, separación de demos, HubSpot fuera de alcance y contrato analítico permanecen intactos.
- `pnpm check`: 7 tareas de lint, 21 tareas de typecheck/test/build y 14 pruebas operativas correctas; 34/34 pruebas del Worker.
- `pnpm e2e`: 65/65 pruebas Chromium correctas. La nueva prueba bloquea `/api/leads`, avanza 15 segundos simulados y verifica botón recuperado, recibo oculto, copia de incertidumbre y reintento con éxito; permanecen verdes axe, reflow a 320 px, texto al 200 %, foco y movimiento reducido.
- QA visual: no cambian estilos ni geometría. El navegador integrado rechazó la URL del proxy local usada para provocar el timeout, por lo que no se obtuvo una captura específica; el estado renderizado y su texto completo sí se verifican dinámicamente en Chromium, junto con toda la regresión responsive y accesible.

## Revisión multidisciplinar del checkpoint actual

- Marketing estratégico: corregido — una conexión bloqueada ya no puede prolongar indefinidamente el envío y aumentar el abandono; la copia evita afirmar una entrega no confirmada.
- Diseño de producto: correcto — se refuerza el único canal real de captación y su recuperación sin introducir otro proveedor, estado persistente o recorrido.
- UX: corregido — el estado de envío completo queda acotado, conserva los campos, recupera el botón y explica un reintento seguro; nunca abre recibo sin entrega obligatoria.
- UI/dirección visual: correcto — el nuevo mensaje usa el estado semántico y el sistema visual existentes; no cambia componentes, estilos ni geometría.
...
- Arquitectura frontend: correcto — el timeout queda encapsulado en el único listener del formulario, limpia su temporizador, reutiliza el contrato actual y no añade dependencias.
- Full stack: correcto — el aborto del cliente queda por encima de los 10 segundos del proveedor, mientras fingerprint, coordinación durable, referencia y claves idempotentes permiten recuperar un acuse perdido.
- QA/accesibilidad/rendimiento/confianza: corregido — 34/34 pruebas del Worker y 65/65 E2E cubren timeout completo, reintento, recibo, axe, foco y responsive; no quedan bloqueantes conocidos.
