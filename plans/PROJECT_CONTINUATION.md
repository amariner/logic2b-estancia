...
- Alcance: transporte de Resend del Worker, prueba unitaria determinista, playbook de recuperación y este checkpoint. No se añadieron dependencias, imágenes, endpoints, proveedores, PII, eventos analíticos, cambios visibles ni despliegues.
- Conversión y resiliencia: cada petición paralela de Resend recibe una señal de aborto a los 10 segundos. Un proveedor bloqueado deja de retener el formulario indefinidamente y vuelve al mismo fallo reintentable o resultado degradado ya visible para la landing.
- Idempotencia y recuperación: el timeout no altera la referencia durable ni las claves `estancia-lead/{ref}/internal` y `estancia-lead/{ref}/visitor`; repetir el payload sin cambios durante 24 horas sigue siendo la recuperación segura ante un acuse perdido.
- Seguridad y privacidad: el aborto se resuelve dentro del transporte, no registra payload, PII, secretos ni respuesta del proveedor. Destinatario, rate limit, consentimiento, demos, HubSpot fuera de alcance y contrato analítico permanecen intactos.
- `pnpm check`: 7 tareas de lint, 21 tareas de typecheck/test/build y 14 pruebas operativas correctas; 34/34 pruebas del Worker. La nueva prueba bloquea ambas llamadas, avanza 10 segundos simulados y verifica señales abortadas más `502 lead_delivery_failed`.
- `pnpm e2e`: 64/64 pruebas Chromium correctas, incluidas semántica negativa del recibo, axe, reflow a 320 px, texto al 200 %, foco y movimiento reducido.
- QA visual: no aplica una captura nueva porque no cambian DOM, estilos ni geometría; la regresión responsive y accesible completa queda verde.

## Revisión multidisciplinar del checkpoint actual

- Marketing estratégico: corregido — un proveedor bloqueado ya no puede prolongar indefinidamente el envío y aumentar el abandono; no se añade ninguna promesa comercial nueva.
- Diseño de producto: correcto — se refuerza el único canal real de captación sin introducir otro proveedor, estado o recorrido.
- UX: corregido — el estado de envío queda acotado y después reutiliza el error reintentable existente; nunca abre un recibo sin entrega obligatoria.
- UI/dirección visual: no aplica — no cambian componentes visibles, copy, estilos ni geometría; los estados ya diseñados se recorrieron sin regresión.
...
- Arquitectura frontend: correcto — el contrato de la landing y su bundle no cambian; el timeout queda encapsulado en el transporte del Worker y no añade dependencias.
- Full stack: corregido — ambas peticiones al proveedor se abortan de forma independiente tras 10 segundos, limpian su temporizador y preservan referencia, claves idempotentes y semántica de entrega parcial.
- QA/accesibilidad/rendimiento/confianza: corregido — 34/34 pruebas del Worker y 64/64 E2E cubren timeout, fallos parciales, reintento, recibo, axe, foco y responsive; no quedan bloqueantes conocidos.

Deuda aceptada: faltan recorridos humanos con VoiceOver y otro lector, modos de alto contraste y validación de comprensión. El timeout, el contrato de entrega y Lighthouse deben comprobarse en producción tras un despliegue autorizado. También queda confirmar el smoke anterior; los textos legales requieren revisión jurídica española; la agenda es opcional y HubSpot continúa fuera de alcance. Las etiquetas/activadores de Estancia dentro del GTM compartido y un periodo agregado posterior a esta nueva línea base bloquean la selección del primer experimento; no se ha inventado variante ni resultado.
