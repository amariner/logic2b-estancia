...
La publicación es manual. El workflow `Deploy production` acepta una confirmación `DEPLOY` sobre `main` o un tag único `deploy-production-*` que apunte exactamente al `HEAD` remoto de `main`; mantiene la exclusión mutua del entorno productivo y vuelve a ejecutar `pnpm check` antes de Wrangler. La alternativa local requiere `wrangler login`. En ambos casos, el Worker `logic-estancia` necesita `LEADS_RESEND_API_KEY`, `LEADS_FROM_EMAIL`, `LEADS_INTERNAL_RECIPIENT` y `LEADS_REPLY_TO`. El destinatario interno de producción es `marinerandreu+logic@gmail.com`. Solo el formulario comercial de la landing llama a `/api/leads`; el diagnóstico enlaza a ese formulario y las demos nunca lo invocan. El Worker no activa el canal de email con configuración parcial. HubSpot permanece intencionadamente fuera de alcance. La configuración versionada crea el Durable Object que mantiene el rate limit y la idempotencia durante el despliegue.
