...
export type EvidenceView = 'home' | 'enquiries' | 'planning' | 'bookings' | 'guests' | 'cleaning' | 'maintenance' | 'channels' | 'control' | 'reports';
...
export const CAPABILITIES: readonly Capability[] = [
  ...
  { id: 'supervised-ai', category: 'ai', minimumPlan: 'inteligente', status: 'demo_visual_pendiente', evidence: { surface: 'none', proof: { es: 'Existe una interfaz interna de borrador, pero no forma parte de la navegación pública actual.', en: 'An internal draft interface exists but is not part of the current public navigation.' }, boundary: { es: 'No debe comunicarse como demostrable; no hay modelo ni proveedor conectado y nada se envía.', en: 'It must not be presented as demonstrable; no model or provider is connected and nothing is sent.' } }, label: { es: 'Copiloto supervisado', en: 'Supervised copilot' }, description: { es: 'Borradores supervisados pendientes de una representación visual pública y validada.', en: 'Supervised drafts pending a validated public visual representation.' } },
