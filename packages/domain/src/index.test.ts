...
describe('domain', () => {
  ...
  it('states the current public evidence for every capability without overstating hidden views', () => {
    expect(Object.fromEntries(CAPABILITIES.map(({ id, status }) => [id, status]))).toEqual({
      ...
      'supervised-ai': 'demo_visual_pendiente',
      revenue: 'en_ruta',
    });
    const exposedWorkspaceViews = new Set(['home', 'enquiries', 'planning', 'bookings', 'guests', 'cleaning', 'maintenance', 'channels', 'control', 'reports']);
